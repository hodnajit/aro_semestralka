...
class FrontierExplorer():
    ...
    def __init__(self):
        # Initialize the node
        rospy.init_node("frontier_explorer_cam")
        ...
        # Subscribe to grid
        self.gridSubscriber = rospy.Subscriber('occupancy_cam', OccupancyGrid, self.grid_cb)
    ...
    def computeWFD(self):
        ...
        im2=np.array([self.gridInfo.width if x==-1 else x for x in tmpGrid])
        for a in frontiers:
            im2[(a[1])*self.gridInfo.width + a[0]] = 75

        im2 = im2.reshape(self.gridInfo.width,self.gridInfo.height)
        self.image = im2
        #plt.imshow(im2)
        #plt.show()
    ...
    def grid_cb(self, msg):
        ...
        if not self.gridReady:
            ...
            # Create services
            self.afl_service = rospy.Service('any_frontiers_left_cam', AnyFrontiersLeft, self.anyFrontiersLeft)
            self.grf_service = rospy.Service('get_random_frontier_cam', GenerateFrontier, self.getRandomFrontier)
            self.gcf_service = rospy.Service('get_closest_frontier_cam', GenerateFrontier, self.getClosestFrontier)
            self.gridReady = True
