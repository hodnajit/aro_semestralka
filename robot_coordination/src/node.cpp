...
//  robot.setPath(path);
  ...
  ros::Publisher cmd_pub = nh.advertise<geometry_msgs::Twist> ("/cmd_vel_mux/safety_controller",10);
  
  /*advertise the topic to publish how many waypoints are ahead of the robot*/
  ros::Publisher state_pub = nh.advertise<std_msgs::Int32> ("waypoints_ahead",10);
  ...
  /*set the state publisher (waypoints ahead)*/
  robot.setStatePublisher(&state_pub);
  /*start execute the path */
